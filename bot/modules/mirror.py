...
from bot import bot, Interval, INDEX_URL, BUTTON_FOUR_NAME, BUTTON_FOUR_URL, BUTTON_FIVE_NAME, BUTTON_FIVE_URL, \
                ...
                LEECH_LOG, SOURCE_LINK, BOT_PM, MIRROR_LOGS, FSUB, CHANNEL_USERNAME, FSUB_CHANNEL_ID
...
class MirrorListener:
            ...
            for dirpath, subdir, files in walk(f'{DOWNLOAD_DIR}{self.uid}', topdown=False):
                for file_ in files:
                    ...
                    if int(f_size) > LEECH_SPLIT_SIZE:
                        ...
                        split_file(f_path, f_size, file_, dirpath, LEECH_SPLIT_SIZE)
                        osremove(f_path)
        if self.isLeech:
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name: str):
        ...
        reply_to = self.message.reply_to_message
        if self.message.chat.type != 'private' != -1:
            if reply_to is not None:
                try:
                    reply_to.delete()
                except Exception as error:
